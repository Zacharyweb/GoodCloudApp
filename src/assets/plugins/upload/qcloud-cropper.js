        ...
        var helper={
            qCloundUpload:function(file,filename){
                cos.uploadFile(function (result) {
                    var prefix = "/" + g.config.appid + "/" + g.config.bucket;
                ...
                }, setting.errorCallBack, setting.progressCallBack, g.config.bucket, (setting.otherFile=="video"?g.config.videoFolder:g.config.myFolder) + filename||file.name, file, 0);
            },
            evalCallbackEle:function(relativePath){
                if(setting.callbackEle){
                    $(setting.callbackEle).each(function(i,item){
                        switch(item.tagName){
                            case "IMG":
                                $(item).attr("src",g.url.imgFormat(relativePath));
                                break;
                            case "INPUT":
            ...
            createModal:function(id,imgSrc){
                var html=[];
                html.push('<div id="upload">');
                html.push('    <div class="up-con">');
                html.push('        <div class="up-co-img">');
                html.push('            <img data-target="images" src="'+imgSrc+'">');
                html.push('           <div data-target="canvasContainer"></div>');
                html.push('        </div>');
                html.push('        <div class="buts">');
                html.push('            <div class="lf" data-func="cancel">取消</div>');
                html.push('            <div class="ri" data-func="save" data-method="getCroppedCanvas">保存</div>');
                html.push('        </div>');
                html.push('    </div>');
                html.push('</div>');
            ...
            if(!setting.cropper||setting.otherFile) {
                ...
                $inputFile.change(function (e) {
                    ...
                    helper.isValid(localUrl,function(){
                        ...
                        $window.find('[data-func="cancel"]').click(function(){
                            $window.remove();
                        });
